// NPRCharacter/Face.shader
// 明日方舟终末地风格卡通脸部 —— 原始 Assets/Resources/Shaders/face.shader (vs_5_0/ps_5_0 asm,1615 行) 的 URP 14.2.0-t1 复刻。
// 变量名按 asm 使用处语义命名、按数据流模块化;URP 有等价物的(变换/主光/附加光衰减与阴影/SH/反射探针)直接使用 URP 内置函数。
// 模块:FaceInput.hlsl(材质参数) / NPRCharacterInput.hlsl(共享参数与工具) / NPRCharacterVertex.hlsl(VS) /
//       NPRCharacterGI.hlsl(环境光) / NPRCharacterLighting.hlsl(共享光照工具) /
//       NPRCharacterFog.hlsl(雾与运动矢量) / FaceRain.hlsl(雨湿润) / FaceLighting.hlsl(材质解码+主光+附加光+输出)。
Shader "NPRCharacter/Face"
{
    Properties
    {
        ...
        _FaceControlTex("Face Control (X=RimStrength Y=Blend Z=WetControl W=RimBoost)", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "Packages/com.unity.render-pipelines.core/ShaderLibrary/CommonMaterial.hlsl"  // LerpWhiteTo(Shadows.hlsl:309 需要,该 pass 不包含 FaceInput)
        ...
        Pass
        {
            ...
            #pragma fragment FaceMetaFrag

            #include "FaceInput.hlsl"
            #include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/UniversalMetaPass.hlsl"

            half4 FaceMetaFrag(Varyings input) : SV_Target
            {
                MetaInput metaInput;
        ...
        Pass
        {
            ...
            #pragma target 3.0
            #pragma vertex CharacterVert
            #pragma fragment FaceFragMotionVector
