...
| Tool | Purpose |
|------|---------|
| `search_code_source` | FTS5 search with history ranking, clustering, scope filtering |
| `get_file_content` | Read file content by line range or anchor |
| `log_code_query` | Record explicit feedback for a query |
...
1. Search with `search_code_source` — returns code snippets, not whole files.
...
4. Use `log_code_query` only to correct automatic feedback.
