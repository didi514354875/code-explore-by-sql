...
1. **`search_unreal_source(query?, raw_query?, module?, limit?)`** — Search with automatic history acceleration.
   - System searches query_logs_fts for similar past queries first, then falls back to full FTS5.
...
- `search_unreal_source` checks history first (query_logs_fts + query_note ranking)
